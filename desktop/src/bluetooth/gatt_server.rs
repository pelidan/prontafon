...
impl GattServer {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let (should_check, is_authenticated) = {
                    ...
                    let check = state_guard.state == ConnectionState::Authenticated && state_guard.device_id.is_some();
                    let auth = state_guard.state == ConnectionState::Authenticated;
                    debug!("BlueZ poll: should_check={}, state={:?}, device_id={:?}", 
                        ...
                        for addr in &addresses {
                            ...
                            if let Ok(device) = adapter.device(*addr) {
                                match device.is_connected().await {
                                    Ok(connected) => {
                                        debug!("BlueZ poll: device {} connected={}", addr, connected);
                                        if connected {
                                            any_connected = true;
                                            break;
                                        }
    ...
    async fn register_gatt_service(&mut self) -> Result<()> {
        ...
        debug!("📝 Registering Command RX characteristic: {}", COMMAND_RX_UUID);
        ...
        debug!("📝 Registering Response TX characteristic: {}", RESPONSE_TX_UUID);
    ...
    async fn handle_command_write(
        ...
        debug!("📥 BLE WRITE RECEIVED: {} bytes, MTU={}, offset={}", 
              data.len(), req.mtu, req.offset);
        ...
        if let Some(complete_message) = state_guard.reassembler.process_packet(&data) {
            debug!("✅ Message reassembly complete: {} bytes", complete_message.len());
