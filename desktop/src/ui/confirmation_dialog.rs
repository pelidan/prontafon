...
pub fn show_confirmation_dialog(
    ...
    let window = ApplicationWindow::builder()
        .application(app)
        .title("Prontafon - Pairing Request")
        .default_width(350)
        .default_height(180)
        .modal(true)
        .resizable(false)
    ...
    reject_button.connect_clicked(move |_| {
        ...
        if let Some(tx) = tx_reject.lock().expect("lock failure").take() {
    ...
    approve_button.connect_clicked(move |_| {
        ...
        if let Some(tx) = tx_approve.lock().expect("lock failure").take() {
    ...
    window.connect_close_request(move |_| {
        ...
        if let Some(tx) = tx_close.lock().expect("lock failure").take() {
            let _ = tx.send(ConfirmationResult::Rejected);
        }
    ...
    glib::timeout_add_seconds_local_once(60, move || {
        ...
        if let Some(tx) = tx_timeout.lock().expect("lock failure").take() {
